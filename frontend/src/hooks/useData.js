...
const useData = () => {
  ...
  const sendMessage = async (value) => axios.post(
    '/api/v1/messages',
    {
      text: value,
  ...
  const sendChannel = async (channelName) => axios.post(
    '/api/v1/channels',
    {
      name: channelName,
    },
  ...
  const deleteChannel = async (id) => axios.delete(`/api/v1/channels/${id}`, { headers });

  const renameChannel = async (id, name) => axios.patch(
    `/api/v1/channels/${id}`,
    {
      id,
      name,
